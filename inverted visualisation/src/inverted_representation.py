...
class InvertedRepresentation():
    def __init__(self, model):
        self.model = model
        self.model.eval()
        if not os.path.exists('../outputs/' + str(target_example)):
            os.makedirs('../outputs/' + str(target_example))
    ...
    def generate_inverted_image_specific_layer(self, input_image, img_size, target_layer=3):
        ...
        for i in range(201):
            ...
            if i % 5 == 0:
                ...
                im_path = '../outputs/' + str(target_example) + '/Inv_Image_Layer_' + str(target_layer) + \
